...
def train_t5_with_deepspeed_fsdp(
    ...
    """
    Train T5 with DeepSpeed and/or FSDP, with checkpoint resumption
    """
    ...
    # Variable to track starting epoch
    start_epoch = 0
    ...
    if use_deepspeed:
        ...
        if resume_from_checkpoint and rank == 0:
            print(f"Loading DeepSpeed checkpoint from {resume_from_checkpoint}")
        ...
        # Load from checkpoint for FSDP if specified
        if resume_from_checkpoint and os.path.isfile(resume_from_checkpoint):
            ...
            if use_fsdp:
                try:
                    ...
                    if resume_scheduler and 'scheduler_state_dict' in checkpoint:
                        lr_scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
                        print("Successfully loaded scheduler state from checkpoint")
                        
                except Exception as e:
                    print(f"Error loading FSDP checkpoint: {e}")
                    print("Will start training from scratch")
            else:
                # Regular DDP checkpoint loading
                ...
                print("Successfully loaded DDP checkpoint")
        ...
        if rank == 0:
            ...
            if use_deepspeed:
                ...
                if avg_loss < best_loss:
                    ...
                    print(f"New best LoRA model saved at {save_path}_best/ with loss: {avg_loss:.4f}")
            ...
            # FSDP checkpoint saving
                ...
                torch.save(checkpoint, f"flan_{save_path}_epoch{epoch+1}.pt")
...
def train_t5_distributed(
    ...
    mp.spawn(
        ...
        args=(world_size, model, dataset, collate_fn, batch_size, use_deepspeed, use_fsdp, 
              num_epochs, accumulation_steps, save_path, resume_from_checkpoint, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume_optimizer', action='store_true', 
                        help='Whether to also load optimizer state from checkpoint')
    parser.add_argument('--resume_scheduler', action='store_true',
                        help='Whether to also load scheduler state from checkpoint')
